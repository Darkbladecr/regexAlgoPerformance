...
const conceptNameSet = new Set(conceptNameList.map((x) => x.toLowerCase()));
const conceptNameArr = [...conceptNameSet];
const conceptNameArrLen = conceptNameArr.length;
...
export default function () {
  ...
  for (let y = 0; y < conceptNameArrLen; y++) {
    const lookup = conceptNameArr[y];
    let startPos = 0;
    while (startPos !== -1) {
      const start = mdLC.indexOf(lookup, startPos);
      if (start === -1) {
        break;
      }
      // console.log("found:", lookup);
      const end = start + lookup.length;
      startPos = end;
      const preU16 = mdLC.charCodeAt(start - 1);
      if (preU16 >= 97 && preU16 <= 122) {
        // console.log("pre throw away:", preU16, md.charAt(start - 1));
        continue;
      }
      const postU16 = mdLC.charCodeAt(end);
      if (postU16 >= 97 && postU16 <= 122) {
        // console.log("post throw away:", postU16, md.charAt(end));
        continue;
      }
      allMatches.push([md.slice(start, end), start, end]);
    }
  }
  const sortedMatches = allMatches.sort((a, b) => b[1] - a[1]);
  ...
  while (incomplete) {
    ...
    solIdxs.push(startPos);
  }
  const solIdxsLen = solIdxs.length - 1;

  let explanation = md;
  for (const [match, start, end] of sortedMatches) {
    let skip = false;
    for (let i = solIdxsLen; i > 0; i--) {
      if (solIdxs[i] < start) {
        const line = md.slice(solIdxs[i], solIdxs[i + 1]);
    ...
    explanation =
      explanation.slice(0, start) +
      `<u data-topicId="{matchingConcept.topicId}" data-conceptId="{matchingConcept.conceptId}" data-chapterId="{matchingConcept.chapterId}">${match}</u>` +
      explanation.slice(end);
  }
  return explanation;
}
