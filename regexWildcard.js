...
const conceptRegex = new RegExp(
  `(.*)\\b(${conceptNameList.join("|")})\\b(.*)`,
  "gi"
);
export default function () {
  const explanation = md.replace(conceptRegex, (match, before, actualText) => {
    if (
      before.startsWith("#") ||
      match.includes("https://") ||
      before.includes('<u data-topicId="')
    ) {
      return match;
    }
    return match.replace(
      actualText,
      `<u data-topicId="{matchingConcept.topicId}" data-conceptId="{matchingConcept.conceptId}" data-chapterId="{matchingConcept.chapterId}">${actualText}</u>`
    );
  });
  return explanation;
}
