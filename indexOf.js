...
export default function () {
  ...
  for (let i = 0; i < linesLen; i++) {
    ...
    for (let y = 0; y < conceptNameArrLen; y++) {
      ...
      const start = lineLC.indexOf(lookup);
      if (start === -1) {
        continue;
      }
      // console.log("found:", lookup);
      const preU16 = lineLC.charCodeAt(start - 1);
      if (preU16 >= 97 && preU16 <= 122) {
        // console.log("pre throw away:", preU16, lineLC.charAt(start - 1));
        continue;
      }
      const end = start + lookup.length;
      const postU16 = lineLC.charCodeAt(end);
      if (postU16 >= 97 && postU16 <= 122) {
        // console.log("post throw away:", postU16, lineLC.charAt(end));
        continue;
      }
      allMatches.push([i, line.slice(start, end), start, end]);
    }
  }
  ...
  let matchIdx = 0;
  let lineIdx = allMatches[matchIdx][0];
  const explanation = lines
    .map((line, index) => {
      if (index !== lineIdx) {
        return line;
      }
      const [_, match, start, end] = allMatches[matchIdx];
      matchIdx++;
      if (allMatches[matchIdx]) {
        lineIdx = allMatches[matchIdx][0];
      } else {
        lineIdx = -1;
      }
      return (
